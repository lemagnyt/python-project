...
def all_data_scraping(players,mode_sleep,test=False):
    '''
    ...
    with(open('data/tennis-data.json'+testString,'w') as Tennis_data_JSON):
...
def matchesData(name,playersDict,mode_sleep,nbMatches=None,test=False):
    '''
    ...
    with(open('data/matches_'+name.replace(' ','_')+testString+'.json','w') as Tennis_data_JSON):
